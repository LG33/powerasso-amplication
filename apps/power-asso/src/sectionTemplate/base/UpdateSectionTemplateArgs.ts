/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { SectionTemplateWhereUniqueInput } from "./SectionTemplateWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { SectionTemplateUpdateInput } from "./SectionTemplateUpdateInput";

@ArgsType()
class UpdateSectionTemplateArgs {
  @ApiProperty({
    required: true,
    type: () => SectionTemplateWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => SectionTemplateWhereUniqueInput)
  @Field(() => SectionTemplateWhereUniqueInput, { nullable: false })
  where!: SectionTemplateWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => SectionTemplateUpdateInput,
  })
  @ValidateNested()
  @Type(() => SectionTemplateUpdateInput)
  @Field(() => SectionTemplateUpdateInput, { nullable: false })
  data!: SectionTemplateUpdateInput;
}

export { UpdateSectionTemplateArgs as UpdateSectionTemplateArgs };
